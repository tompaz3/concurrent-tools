...
class TryExecuteTest {
  ...
  void testValuesLeftIdentity() {
    // given
    final Function<Integer, TryExecute<String, Integer>> flatMapper = i -> i < 10
    ...
    // when
    final TryExecute<String, Integer> tryExecute = TryExecute.<String, Integer>of(value)
        .flatMap(flatMapper);
    final TryExecute<String, Integer> tryExecute2 = TryExecute.of(expectedValue);
  ...
  void testValuesRightIdentity() {
    // given
    final TryExecute<String, Integer> value = TryExecute.of(20);

    // when
    final TryExecute<String, Integer> tryExecute = value.flatMap(TryExecute::of);
  ...
  void testValuesAssociativityIdentity() {
    // given
    final Function<Integer, TryExecute<String, Integer>> flatMapper = i -> i < 15
        ? TryExecute.ofError("Lower than 15")
        : TryExecute.of(i * 2);
    final Function<Integer, TryExecute<String, Integer>> flatMapper2 = i -> i > 30
        ? TryExecute.of(i + 1)
        : TryExecute.ofError("Lower than 30");
    final TryExecute<String, Integer> value = TryExecute.of(20);

    // when
    final TryExecute<String, Integer> tryExecute = value
        .flatMap(flatMapper)
        .flatMap(flatMapper2);
    final TryExecute<String, Integer> tryExecute2 = value
  ...
  void testErrorsLeftIdentity() {
    ...
    final Function<String, TryExecute<String, Integer>> flatMapper = e -> e.contains("error")
        ? TryExecute.ofError(expectedMsg)
        : TryExecute.of(5);

    // when
    final TryExecute<String, Integer> tryExecute = TryExecute.<String, Integer>ofError(
        errorMsg)
        .flatMapError(flatMapper);
    final TryExecute<String, Integer> tryExecute2 = TryExecute.ofError(expectedMsg);
  ...
  void testErrorsRightIdentity() {
    // given
    final var errorMsg = "This is an error";
    final TryExecute<String, Integer> value = TryExecute.ofError(errorMsg);
  ...
  void testErrorsAssociativityIdentity() {
    ...
    final Function<String, TryExecute<String, Integer>> flatMapper = e -> e.contains("error")
        ? TryExecute.ofError(expectedMsg)
        : TryExecute.of(5);
    final Function<String, TryExecute<String, Integer>> flatMapper2 = e -> expectedMsg.equals(e)
